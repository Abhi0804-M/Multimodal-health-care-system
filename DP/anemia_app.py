...
# Initialize Flask app with proper static/template folders
app = Flask(__name__, 
...
def index():
    ...
    if is_mobile():
        ...
        k=1
        print(k)
    return render_template('index.html'), 200, {'X-Accel-Buffering': 'no'}

app.config['SEND_FILE_MAX_AGE_DEFAULT'] = 0
app.config['TEMPLATES_AUTO_RELOAD'] = True
# Get the directory where this script is located

model_path_Vitamin = os.path.join(BASE_DIR, 'vitamin_deficiency_model.h5')
model_path_Iron = os.path.join(BASE_DIR, 'iron_deficiency_model.h5')
model_path_sickle = os.path.join(BASE_DIR, 'sickle_cell_model.h5')
model_path_anemia = os.path.join(BASE_DIR, 'anemia_model.h5')

iron_deficiency_model = load_model(model_path_Iron)
vitamin_deficiency_model = load_model(model_path_Vitamin)
sickle_cell_model = load_model(model_path_sickle)
anemia_model= load_model(model_path_anemia)

scaler = MinMaxScaler()
scaler.fit([[80, 27, 11], [100, 30, 11]])
# Load the anemia prediction models
model_path_Conjuctiva = os.path.join(BASE_DIR, 'conjunctiva_100.h5')
model_path_Nail = os.path.join(BASE_DIR, 'fingerNail_100.h5')
model_path_Palm = os.path.join(BASE_DIR, 'palm_100.h5')
anemia_eye_model = load_model(model_path_Conjuctiva)
anemia_nail_model = load_model(model_path_Nail)
anemia_palm_model = load_model(model_path_Palm)
...
# Replace the URL with the video stream link
        ...
        if results.multi_hand_landmarks:
            for hand_landmarks in results.multi_hand_landmarks:
                ...
                if not palmar_side and start_time is not None and time.time() - start_time >= capture_time:
                    ...
                    cv2.imwrite(os.path.join(BASE_DIR, 'PalmDetection','captured_photos', 'palm_region.png'), palm_region)
    ...
    image_path =os.path.join(BASE_DIR, 'PalmDetection','captured_photos', 'palm_region.png')
    ...
    if contours:
        ...
        polygon_cutout_path = os.path.join(BASE_DIR, 'captured_photos', 'palm_transparent.png')
    ...
    result1= predict_anemia(anemia_palm_model, os.path.join(BASE_DIR, 'captured_photos', 'palm_transparent.png'), image_size=(128, 128))
...
# Display the image
    ...
    # Create a binary mask where non-black areas are white
        ...
        if contours:
            ...
            polygon_cutout_rgba[black_pixels | green_pixels, 3] = 0
            # Save the final transparent cutout
            polygon_cutout_path = os.path.join(BASE_DIR, 'captured_photos', 'palm_transparent1.png')
            cv2.imwrite(polygon_cutout_path, polygon_cutout_rgba)
            plt.imshow(cv2.cvtColor(polygon_cutout_rgba, cv2.COLOR_BGRA2RGBA))
            plt.axis("off")
            plt.show()
        image_path= os.path.join(BASE_DIR, 'captured_photos', 'palm_transparent1.png')
...
def upload():
    if request.method == 'POST':
        ...
        if file:
            ...
            rf = Roboflow(api_key="AYaR4eY0pFr3ZEjLwR5w")
            project = rf.workspace().project("nail-bheyh")
    ...
    while current_try < max_retries:
        try:
            if is_port_in_use(port):
                print(f"Port {port} is in use, trying to kill existing process...")
                import subprocess
            ...
            current_try += 1
            print(f"Attempt {current_try}: Error starting Flask app: {e}")
            if current_try == max_retries:
                print("Failed to start Flask app after maximum retries")
                sys.exit(1)
            time.sleep(2)
